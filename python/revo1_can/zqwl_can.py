...
from zqwl_win import zcan_open, zcan_close, zcan_send_message, revo1_can_receive_message
...
class Revo1CanController:
    ...
    def _can_read(self, slave_id: int) -> tuple:
        """CAN消息接收"""
        try:
            recv_msg = revo1_can_receive_message()
            if recv_msg is None:
                return 0, bytes([])
    ...
    async def demo_task(self):
        ...
        await self.single_finger_control_example(libstark.FingerId.Index)
