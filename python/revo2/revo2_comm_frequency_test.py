...
本测试专门用于评估Revo2灵巧手的实际使用场景下的通讯频率和性能，包括：
...
class Revo2CommTester:
    ...
    async def test_set_finger_positions_frequency(self) -> FrequencyTestReport:
        ...
        position_sequences = [
            [400, 400, 1000, 1000, 1000, 1000],  # 握手
            [400, 400, 0, 0, 0, 0],              # 张开
            [400, 400, 500, 500, 500, 500],      # 半握
            [400, 400, 0, 0, 0, 0],              # 张开
        ]
    ...
    async def test_mixed_frequency(self) -> FrequencyTestReport:
        ...
        position_sequences = [
            [400, 400, 0, 0, 0, 0],              # 张开
            [400, 400, 300, 300, 300, 300],      # 预备位置
            [400, 400, 600, 600, 600, 600],      # 接触
            [400, 400, 1000, 1000, 1000, 1000],  # 握紧
        ]
    ...
    def save_detailed_results(self, reports: List[FrequencyTestReport], stability_data: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"📁 详细测试结果已保存到: revo2_comm_test_report_{timestamp}.json")

async def main():
    """主测试函数"""
    print("🤖 Revo2灵巧手通讯频率测试")
    print("="*60)
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print("✅ 所有测试完成！")
        print("📊 结果图表已保存为: revo2_comm_frequency_test_results.png")
        print(f"{'='*60}")
