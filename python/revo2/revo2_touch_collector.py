"""
Revo2 Touch (Capacitive) - High-Performance Data Collector Example with Trajectory Control
...
Hardware Support:
- ✅ Revo2 Touch (Capacitive Touch Sensor)

Usage:
    python revo2_touch_collector.py
"""
...
async def main():
    ...
    if client.is_touch_pressure(slave_id):
        logger.error("This example is for capacitive touch sensors!")
        logger.error("Current device has pressure touch sensor (Modulus)")
        logger.error("Please use revo2_touch_pressure_buffer.py instead")
        sys.exit(1)
    ...
    motor_frequency = 200 if is_linux else 20  # Motor frequency
    touch_frequency = 200 if is_linux else 20  # Touch frequency
...
async def process_motor_data(motor_buffer, frequency, index_ref=None):
    ...
    while True:
        try:
            ...
            if len(data_batch) > 0:
                if process_count % 20 == 0:  # Print every 20 times
                    traj_info = f" [Traj Index: {index_ref['value']}]" if index_ref else ""
                    logger.info(f"[{process_count}]{traj_info} Processed {len(data_batch)} motor data")
        ...
        try:
            ...
            for finger_idx in range(5):
                if len(all_data[finger_idx]) > 0:
                    # Get latest data
                    latest_item = all_data[finger_idx][-1]
