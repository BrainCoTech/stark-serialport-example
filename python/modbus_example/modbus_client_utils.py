...
def client_connect(port: str, baudrate: BaudRate, broadcast_enable=False) -> ModbusSerialClient:
    client = ModbusSerialClient(port=port, baudrate=baudrate.value, broadcast_enable=broadcast_enable, timeout=3)
    if not is_connected(client):
        raise ValueError(f"Failed to connect to Modbus client on port {port} at baudrate {baudrate}")
    return client
...
def client_write_registers(client: ModbusSerialClient, address: int, values: list, slave: int)-> int:
    SKLog.debug(f"client_write_registers, address: {address}, len={len(values)}, {hexlify_packets(values)}")
    ...
    if isinstance(client, ModbusSerialClient) and is_connected(client):
        try:
            if isinstance(values, list): 
                ...
                SKLog.warning(f"client_write_registers, values type error: {type(values)}")     
            return 0    
        except Exception as e:
...
def client_read_holding_registers(client: ModbusSerialClient, register_address: int, count: int = 1, slave: int = 0) -> list:
    SKLog.debug(f"read_holding_registers, register_address: {register_address}, count: {count}")

    if isinstance(client, ModbusSerialClient) and is_connected(client):
        try:
            response = client.read_holding_registers(register_address, count=count, slave=slave)
            values = response.registers
            SKLog.debug(f"read_holding_registers, register_address: {register_address}, len={len(values)}, {hexlify_packets(values)}")
            return values
        except Exception as e:
...
def client_read_input_registers(client: ModbusSerialClient, register_address: int, count: int = 1, slave: int = 0) -> list:      
    SKLog.debug(f"read_input_registers, register_address: {register_address}, count: {count}")

    if isinstance(client, ModbusSerialClient) and is_connected(client):
        try:
            response = client.read_input_registers(register_address, count=count, slave=slave)
            values = response.registers
            SKLog.debug(f"read_input_registers, register_address: {register_address}, len={len(values)}, {hexlify_packets(values)}")
            return values
        except Exception as e:
