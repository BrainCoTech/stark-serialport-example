...
class AutoDetectWorker(QObject):
    ...
    async def _auto_detect(self):
        ...
        # Get full device info for GUI display
...
class ConnectionPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.protocol_combo.addItems(["Auto Detect", "Modbus/RS485", "Protobuf", "CAN 2.0", "CANFD"])
    ...
    def _on_disconnect(self):
        ...
        if self.ctx:
            try:
                # Use unified close_device_handler for all protocols
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                try:
