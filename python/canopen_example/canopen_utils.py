...
def connect_to_canopen_node(node_id: int, interface: str = None, channel: int | str = None, bitrate: int = 0) -> tuple[canopen.Network, canopen.RemoteNode]:
    ...
    node = canopen.RemoteNode(node_id, 'finger123456.eds')
    ...
    # Re-map TPDO[1]
    node.tpdo[1].clear()
    node.tpdo[1].add_variable('ActualPosition', 0)
    ...
    node.tpdo[1].event_timer = 10  # 设置事件计时器为10毫秒
    node.tpdo[1].enabled = True
    node.tpdo[1].save() # Save new PDO configuration to node

    # Re-map TPDO[2]
    node.tpdo[2].clear()
    node.tpdo[2].add_variable('AccturalPosition', 4)
    node.tpdo[2].add_variable('AccturalPosition', 5)
    node.tpdo[2].trans_type = 254  # 设置传输类型为周期性传输
    node.tpdo[2].event_timer = 10  # 设置事件计时器为10毫秒
    node.tpdo[2].enabled = True
    node.tpdo[2].save() # Save new PDO configuration to node

    # Re-map RPDO[1]
    node.rpdo[1].clear()
    node.rpdo[1].add_variable('TargetPosition', 0)
    ...
    # Re-map RPDO[2]
    node.rpdo[2].clear()
    node.rpdo[2].add_variable('TargetPosition', 4)
...
def get_finger_position(node: canopen.RemoteNode, id: FingerId, wait_for_reception: bool = False):
    ...
    if idx <= 3:
        ...
        actual_position = node.tpdo[1]['ActualPosition.' + str(idx)].raw
    else:
        if wait_for_reception:
            node.tpdo[2].wait_for_reception()
        actual_position = node.tpdo[2]['ActualPosition.' + str(idx)].raw
...
def get_finger_positions(node: canopen.RemoteNode, wait_for_reception: bool = False):
    ...
    actual_positions = [
        node.tpdo[1]['ActualPosition.0'].raw,
        node.tpdo[1]['ActualPosition.1'].raw, 
        node.tpdo[1]['ActualPosition.2'].raw,
        node.tpdo[1]['ActualPosition.3'].raw,
        node.tpdo[2]['ActualPosition.4'].raw,
        node.tpdo[2]['ActualPosition.5'].raw
    ]
...
def set_finger_position(node: canopen.RemoteNode, id: FingerId, position: int):
    idx = id.value - 1
    if idx <= 3:
        node.rpdo[1]['TargetPosition.' + str(idx)].raw = position
    else:
        node.rpdo[2]['TargetPosition.' + str(idx)].raw = position
