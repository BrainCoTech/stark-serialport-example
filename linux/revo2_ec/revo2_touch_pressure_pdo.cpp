...
ec_pdo_entry_info_t slave_pdo_entries[] = {
    ...
    {0x6010, 0x06, 240},  // force_palm (46 x uint16)， TODO: 固件目前为47*16
    {0x0000, 0x00, 240},  // Gap
    {0x0000, 0x00, 240},  // Gap
    {0x0000, 0x00, 32},   // Gap
...
ec_pdo_info_t slave_pdos[] = {
    ...
    {0x1A01, 10, &slave_pdo_entries[11]}, // TxPDO: 0x6010:01-07 触觉反馈
};
...
/****************************************************************************/
// 压感式触觉反馈数据读取函数
// ● 拇指力阵列（force_thumb） → UINT16 ×9
// ● 食指力阵列（force_index） → UINT16 ×9
// ● 中指力阵列（force_mid） → UINT16 ×9
// ● 无名指力阵列（force_ring） → UINT16 ×9
// ● 小指力阵列（force_pinky） → UINT16 ×9
// ● 手掌力阵列（force_palm） → UINT16 ×46
// ● 合力：拇指、食指、中指、无名指、小指、手掌（force_total） → UINT16 ×6
/****************************************************************************/

// 触摸反馈数据结构常量定义
#define FINGER_SENSOR_COUNT     9    // 每个手指的传感器数量
#define PALM_SENSOR_COUNT       46   // 手掌传感器数量
#define FINGER_TOTAL_COUNT      6    // 合力数据的手指数量
...
#define FINGER_TOTAL_OFFSET     (230 + 2)  // TODO: 固件多了2个字节
...
static bool is_data_boundary_valid(size_t offset, size_t size) {
    return (offset + size) <= (off_in + 512); // 假设最大域大小为512字节
}
...
void read_touch_feedback_data() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        // 分配数据缓冲区
        uint8_t finger_data[PALM_DATA_SIZE]; // 使用最大尺寸作为缓冲区
