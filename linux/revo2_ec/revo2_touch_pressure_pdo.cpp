...
ec_pdo_entry_info_t slave_pdo_entries[] = {
    ...
    // TxPDO: 0x1A01 (slave -> master) pressure‑sensing tactile data
    ...
    {0x6010, 0x06, 240},  // force_palm (46 x uint16), TODO: firmware currently uses 47*16
    {0x0000, 0x00, 240},  // Gap
    {0x0000, 0x00, 240},  // Gap
    {0x0000, 0x00, 32},   // Gap
...
ec_pdo_info_t slave_pdos[] = {
    ...
    {0x1A01, 10, &slave_pdo_entries[11]}, // TxPDO: 0x6010:01-07 tactile feedback
};
...
/****************************************************************************/
// High‑level gesture control functions
/****************************************************************************/

// Set thumb control
void set_thumb_control(uint16_t flex_pos, uint16_t abduct_pos,
                       uint16_t duration) {
  set_single_joint_position_time(THUMB, flex_pos, duration);
  set_single_joint_position_time(THUMB_AUX, abduct_pos, duration);
...
void set_ok_gesture(uint16_t duration) {
  uint16_t ok_positions[6] = {400, 400, 700,
                              100, 100, 100}; // thumb and index contact, others open
...
// Configure trajectory point sequence (supports multi‑segment trajectories)
...
void update_trajectory() {
  ...
  // Check whether each joint needs a trajectory update
  bool any_joint_active = false;

  for (int i = 0; i < 6; i++) {
    // Check whether the current joint's trajectory is finished
    if (elapsed_time < traj_ctrl.durations[i]) {
      any_joint_active = true;

      // Compute the current desired position (linear interpolation)
  ...
  // If all joint trajectories are finished, stop trajectory control
  if (!any_joint_active) {
    traj_ctrl.trajectory_active = false;
...
// Advanced trajectory update function (supports multi‑segment trajectories)
void update_advanced_trajectory() {
  ...
  // Check whether the entire trajectory sequence is finished
...
/****************************************************************************/
// Pressure‑sensing tactile feedback read function
// ● Thumb force array (force_thumb)   → UINT16 ×9
// ● Index force array (force_index)   → UINT16 ×9
// ● Middle force array (force_mid)    → UINT16 ×9
// ● Ring force array (force_ring)     → UINT16 ×9
// ● Pinky force array (force_pinky)   → UINT16 ×9
// ● Palm force array (force_palm)     → UINT16 ×46
// ● Resultant forces: thumb, index, middle, ring, pinky, palm (force_total) → UINT16 ×6
/****************************************************************************/

// Tactile feedback data structure constants
#define FINGER_SENSOR_COUNT     9    // number of sensors per finger
#define PALM_SENSOR_COUNT       46   // number of palm sensors
#define FINGER_TOTAL_COUNT      6    // number of fingers in resultant force data

#define FINGER_DATA_SIZE        (FINGER_SENSOR_COUNT * sizeof(uint16_t))  // 18 bytes
#define PALM_DATA_SIZE          (PALM_SENSOR_COUNT * sizeof(uint16_t))    // 92 bytes
#define FINGER_TOTAL_DATA_SIZE  (FINGER_TOTAL_COUNT * sizeof(uint16_t))   // 12 bytes

// Data offset definitions
#define THUMB_OFFSET            48
#define INDEX_OFFSET            66
#define MIDDLE_OFFSET           84
#define RING_OFFSET             102
#define PINKY_OFFSET            120
#define PALM_OFFSET             138
#define FINGER_TOTAL_OFFSET     (230 + 2)  // TODO: firmware has 2 extra bytes

// Finger name array for loop processing
static const char* finger_names[] = {
    "Thumb", "Index", "Middle", "Ring", "Pinky", "Palm"
};

// Per‑finger data size array
static const size_t finger_data_sizes[] = {
    FINGER_DATA_SIZE, FINGER_DATA_SIZE, FINGER_DATA_SIZE,
    FINGER_DATA_SIZE, FINGER_DATA_SIZE, PALM_DATA_SIZE
};

// Per‑finger data offset array
static const size_t finger_offsets[] = {
    THUMB_OFFSET, INDEX_OFFSET, MIDDLE_OFFSET,
    RING_OFFSET, PINKY_OFFSET, PALM_OFFSET
};

// Check whether data boundaries are valid
static bool is_data_boundary_valid(size_t offset, size_t size) {
    return (offset + size) <= (off_in + 512); // assume maximum domain size is 512 bytes
}

void read_touch_feedback_data() {
    // Check input validity
    if (!domain_data) {
        printf("Error: domain_data is NULL\n");
        return;
    }

    printf("===========================\n");

    // Process all finger data in a loop
    for (int i = 0; i < 6; i++) {
        // Check data boundaries
        if (!is_data_boundary_valid(off_in + finger_offsets[i], finger_data_sizes[i])) {
            printf("Error: Data boundary check failed for %s\n", finger_names[i]);
            continue;
        }

        // Allocate data buffer
        uint8_t finger_data[PALM_DATA_SIZE]; // use maximum size as buffer

        // Copy data
        memcpy(finger_data, domain_data + off_in + finger_offsets[i], finger_data_sizes[i]);

        // Print data
        // printf("%s Force: ", finger_names[i]);
        // print_hex(finger_data, finger_data_sizes[i]);
    }

    // Process resultant force data
    if (is_data_boundary_valid(off_in + FINGER_TOTAL_OFFSET, FINGER_TOTAL_DATA_SIZE)) {
        uint8_t force_total[FINGER_TOTAL_DATA_SIZE];
        memcpy(force_total, domain_data + off_in + FINGER_TOTAL_OFFSET, FINGER_TOTAL_DATA_SIZE);

        printf("Force Total: ");
        for (size_t i = 0; i + 1 < FINGER_TOTAL_DATA_SIZE; i += 2) {
            uint16_t value = (uint16_t)force_total[i] | ((uint16_t)force_total[i + 1] << 8);
            printf("%u ", value);
        }
        printf("\n");
    } else {
        printf("Error: Data boundary check failed for Finger Total\n");
    }
  ...
  while (1) {
    ...
    if (master_state.al_states == 0x08) {
      ...
      if (demo_counter++ % (FREQUENCY * 20) == 0) {
        switch (demo_stage % 8) { // include gesture demos
        case 0: {
