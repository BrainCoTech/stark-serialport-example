...
// 辅助函数：解析turbo参数
void parse_turbo_param(uint32_t turbo_param, uint16_t *interval,
                       uint16_t *duration) {
...
int read_sdo_uint8(uint16_t index, uint8_t subindex, uint8_t *value) {
  ...
  if (result_size != sizeof(uint8_t)) {
    printf("Unexpected data size for SDO 0x%04X:0x%02X - Expected: %zu, Got: "
           "%zu\n",
...
int read_general_config(general_config_t *config) {
  ...
  // 读取Turbo参数（32位，大端序）
  ret |= read_sdo_uint32(CONFIG_OBJECT_INDEX, TURBO_PARAM_SUBINDEX,
                         &config->turbo_param);
