...
// This file implements the Windows CANFD example using the ZLG Windows API
// (`zlgcan.dll`), aligned with the Python implementation in
// `python/revo2_canfd/zlg_win.py`.

// Global ZLG handles
static DEVICE_HANDLE   g_device_handle  = INVALID_DEVICE_HANDLE;
static CHANNEL_HANDLE  g_channel_handle = INVALID_CHANNEL_HANDLE;
...
/****************************************************************************/
// CANFD Functions (ZLG Windows API, aligned with zlg_win.py)
/****************************************************************************/

// ZLG device constants (see zlgcan documentation / Python zlg_win.py)
...
// Initialize CANFD device (open ZLG device)
static bool init_canfd_device(int device_type, int device_index) {
  ...
  printf("ZCAN_OpenDevice, device_handle: %p\n", g_device_handle);
  return true;
}
...
static bool setup_canfd_baudrate(int channel_index, int abit_baud, int dbit_baud) {
  ...
  strcpy_s(value, "0");
  if (ZCAN_SetValue(g_device_handle, path, value) != STATUS_OK) {
    printf("Failed to clear filter: %s\n", path);
    return false;
  }

  // Enable/disable merge receive (similar to Python zlg_win.py)
  sprintf_s(path, "%d/set_device_recv_merge", channel_index);
  strcpy_s(value, "0"); // 0 - disable, 1 - enable
  if (ZCAN_SetValue(g_device_handle, path, value) != STATUS_OK) {
    printf("Failed to set %s\n", path);
    return false;
  }
...
static bool start_canfd_channel(int channel_index) {
  ...
  printf("ZCAN_InitCAN\n");
  g_channel_handle = ZCAN_InitCAN(g_device_handle, channel_index, &cfg);
  if (g_channel_handle == INVALID_CHANNEL_HANDLE) {
    printf("Failed to initialize CANFD channel\n");
    return false;
  }

  printf("ZCAN_StartCAN\n");
...
static void setup_canfd_callbacks(void) {
  ...
  set_can_tx_callback([](uint8_t slave_id,
    ...
    printf("[CANFD TX] slave_id=%u can_id=0x%08X len=%zu\n",
           slave_id, can_id, (size_t)data_len);

    ZCAN_TransmitFD_Data tx{};

    // Configure CAN ID as extended frame:
    // bit 31 = EFF flag, bit 30 = RTR, bit 29 = ERR, lower 29 bits = ID
    tx.frame.can_id = (can_id & 0x1FFFFFFF) | (1u << 31); // Extended frame
    tx.frame.len    = (data_len > 64) ? 64 : (BYTE)data_len;

    // flags: bit0 = BRS (bit-rate switch)
    tx.frame.flags = 0;
    tx.frame.flags |= 0x1; // Enable BRS
    ...
    UINT sent = ZCAN_TransmitFD(g_channel_handle, &tx, 1);
    if (sent != 1) {
      printf("[CANFD TX][ERROR] ZCAN_TransmitFD failed, sent=%u\n", sent);
      return -1;
    }

    printf("[CANFD TX] success, sent=%u\n", sent);
    return 0;
  });
  ...
  set_can_rx_callback([](uint8_t slave_id,
    ...
    if (pending == 0) {
      printf("[CANFD RX] slave_id=%u, pending=0 (no frames)\n", slave_id);
      return -1;
    }
    ...
    printf("[CANFD RX] slave_id=%u, pending=%u\n", slave_id, pending);

    UINT received = ZCAN_ReceiveFD(g_channel_handle, rcv, pending, RX_WAIT_TIME);
    if (received == 0) {
      printf("[CANFD RX][ERROR] ReceiveFD returned 0 frames\n");
      return -1;
    }

    printf("[CANFD RX] received=%u\n", received);

    // Prefer the last non-TX-echo frame (similar to Python implementation)
    const ZCAN_ReceiveFD_Data *last_rx = nullptr;
    for (int i = (int)received - 1; i >= 0; --i) {
      const canfd_frame &frame = rcv[i].frame;
      // TX echo uses flag bit5 = 1; skip those
      if (frame.flags & 0x20) {
        printf("[CANFD RX] frame[%d] is TX echo, id=0x%08X len=%u\n",
    ...
    if (last_rx == nullptr) {
      ...
      printf("[CANFD RX][WARN] all %u frames are TX echo, no RX data\n", received);
      return -1;
    }
    ...
    *data_len_out  = frame.len;
    ...
    printf("[CANFD RX] picked frame: can_id=0x%08X len=%u\n",
...
void cleanup_can_resources(void) {
  if (g_channel_handle != INVALID_CHANNEL_HANDLE) {
    ...
    g_channel_handle = INVALID_CHANNEL_HANDLE;
  }
  if (g_device_handle != INVALID_DEVICE_HANDLE) {
...
bool setup_canfd(void) {
  printf("Setting up CANFD (ZLG Windows)...\n");
